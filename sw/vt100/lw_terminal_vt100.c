...
#define SCREEN_PTR(vt100, x, y)                                 \
   Rem ((vt100->top_line * vt100->width * CHRW + x * CHRW + vt100->width * CHRW * y)    \
     , (vt100->width * CHRW * SCROLLBACK * vt100->height))

#define FROZEN_SCREEN_PTR(vt100, x, y)              \
    Rem ((x * CHRW + vt100->width * CHRW * y)                         \
     , (vt100->width * CHRW * SCROLLBACK * vt100->height))
...
char *pdst;
    if (y < headless_term->margin_top || y > headless_term->margin_bottom)
        pdst = &headless_term->frozen_screen[FROZEN_SCREEN_PTR(headless_term, x, y)];
    else
        pdst = &headless_term->screen[SCREEN_PTR(headless_term, x, y)];
	*pdst++ = c;
	*pdst++ = headless_term->color;
}
...
char *pdst;
    if (y < vt100->margin_top || y > vt100->margin_bottom)
        pdst = &vt100->frozen_screen[FROZEN_SCREEN_PTR(vt100, x, y)];
    else
        pdst = &vt100->screen[SCREEN_PTR(vt100, x, y)];
	return *pdst;
}

static void froze_line(struct lw_terminal_vt100 *vt100, unsigned int y)
{
    memcpy(vt100->frozen_screen + vt100->width * CHRW * y,
...
static void unfroze_line(struct lw_terminal_vt100 *vt100, unsigned int y)
{
    memcpy(vt100->screen + SCREEN_PTR(vt100, 0, y),
           vt100->frozen_screen + vt100->width * CHRW * y,
           vt100->width * CHRW);
}
...
/*
  ...
  Chapter 1).
*/
void set_color(struct lw_terminal_vt100 *vt100,unsigned char clr)
{
unsigned char color;
        if(clr>=30 && clr<38)
        {
        	//foreground color
        	color=(clr-30)&7;
        	color=((color&1)<<2) | (color&2) | ((color&4)>>2);
        	vt100->color = (vt100->color&0xF0) | color;
        }
        else
        if(clr>=40 && clr<48)
        {
        	...
        	color=(clr-40)&7;
        	color=((color&1)<<2) | (color&2) | ((color&4)>>2);
        	vt100->color = (vt100->color&0x0F) | (color<<4);
        }
        else
        //if(clr<8)
        	vt100->color = 0x07; //reset color
}
...
static void SGR(struct lw_terminal *term_emul)
{
    ...
    if (term_emul->argc > 0)
    {
        ...
        set_color(vt100,arg0);
    }
    if (term_emul->argc > 1)
    {
        arg1 = term_emul->argv[1];
        set_color(vt100,arg1);
    }
}
...
static void IND(struct lw_terminal *term_emul)
{
    ...
    if (vt100->y >= vt100->margin_bottom)
    {
        ...
        vt100->top_line = /*(vt100->top_line + 1) % (vt100->height * SCROLLBACK);*/
        	Rem( (vt100->top_line + 1) , (vt100->height * SCROLLBACK) );
        for (x = 0; x < vt100->width; ++x)
                                     ...
                                     void (*unimplemented)(struct lw_terminal* term_emul, char *seq, char chr))
{
    ...
    this = (struct lw_terminal_vt100*)calloc(1, sizeof(*this));
    if (this == NULL)
        return NULL;
    ...
    this->height = 24;
    this->width = 80;
    this->screen = calloc(1,132 * CHRW * SCROLLBACK * this->height);
    if (this->screen == NULL)
        goto free_this;
    //memset(this->screen, ' ', 132 * CHRW * SCROLLBACK * this->height);
    this->frozen_screen = calloc(1,132 * CHRW * this->height);
    if (this->frozen_screen == NULL)
        goto free_screen;
    //memset(this->frozen_screen, ' ', 132 * CHRW * this->height);
    this->tabulations = calloc(1,132 * CHRW );
