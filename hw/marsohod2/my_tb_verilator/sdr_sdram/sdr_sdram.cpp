...
SDRAM::SDRAM(vluint8_t log2_rows, vluint8_t log2_cols, vluint8_t flags, char *logfile)
{
    ...
    else
    {
        ...
        for (int i = 0; i < SDRAM_NUM_BANKS; i++)
        {
            for (int j = 0; j < s; j++)
            {
                                      mem_array_0[i][j] = (vluint8_t)0x55;
                if (flags & DATA_MSB) mem_array_1[i][j] = (vluint8_t)0x55;
                if (flags & DATA_MSW) mem_array_2[i][j] = (vluint8_t)0x55;
                if (flags & DATA_MSW) mem_array_3[i][j] = (vluint8_t)0x55;
                if (flags & DATA_MSL) mem_array_4[i][j] = (vluint8_t)0x55;
                if (flags & DATA_MSL) mem_array_5[i][j] = (vluint8_t)0x55;
                if (flags & DATA_MSL) mem_array_6[i][j] = (vluint8_t)0x55;
                if (flags & DATA_MSL) mem_array_7[i][j] = (vluint8_t)0x55;
            }
        }
    }
}
...
// Mem file loading
void SDRAM::loadMem( const char *name )
{
	int n=0;
	unsigned int min_addr=0xffffff;
	unsigned int max_addr=0x000000;

	cout << "Load mem file: " << name << "\n";
	...
	{
		if( str.at(0)=='/' ) continue; //should be comment
		if( str.at(0)=='@' )
		{
			//should be addr-val pair
			size_t found = str.find(" ");
			if( found!=std::string::npos )
			{
				string addr = str.substr(1,found-1);
				string val  = str.substr(found+1);
				unsigned int x_addr = std::stol("0x"+addr, 0, 16) & 0x00FFFFFF;
				unsigned int x_val  = std::stol("0x"+val,  0, 16);
				n++;
				if( n<10 )
				    cout << "Mem-" << addr << "-" << val << "-\n";
				min_addr = min(min_addr,x_addr);
				max_addr = max(max_addr,x_addr );
				memory[x_addr+0] = (x_val    ) & 0xFF;
				memory[x_addr+1] = (x_val>>8 ) & 0xFF;
				memory[x_addr+2] = (x_val>>16) & 0xFF;
				memory[x_addr+3] = (x_val>>24) & 0xFF;
			}
		}
	}

	int sz = max_addr - min_addr + 4;

	//save to binary file
	ofstream wf("tmp.dat", ios::out | ios::binary);
	wf.write( (char *) &memory[min_addr], sz );
	wf.close();

	delete[] memory;

	//load as binary
	load( "tmp.dat", sz, min_addr );
}
